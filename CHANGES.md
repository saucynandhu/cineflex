...
### Changes Made:
- Added global adult content filtering to all TMDB API calls (`include_adult: false`).
...
- (Experimental) Implemented and subsequently removed Mood and Decade filters as per design iterations.
...
- src/pages/Home.tsx — Cleaned up mood filter logic (removed).
- src/pages/Movies.tsx — Cleaned up decade filter logic (removed).
